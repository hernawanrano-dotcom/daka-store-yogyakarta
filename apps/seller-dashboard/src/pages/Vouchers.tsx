...
export default function SellerVouchersPage() {
  ...
  const fetchVouchers = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/vouchers/seller/my`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/vouchers/seller`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/vouchers/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      const token = localStorage.getItem('accessToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/vouchers/${id}/toggle`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        ...
        <main className="p-6">
          ...
          {filteredVouchers.length === 0 && (
            <div className="bg-white rounded-xl shadow-sm p-12 text-center">
              ...
              <p className="text-gray-500">Belum ada voucher toko</p>
              <button
